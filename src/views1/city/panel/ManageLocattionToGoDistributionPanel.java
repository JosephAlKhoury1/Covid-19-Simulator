...
public class ManageLocattionToGoDistributionPanel extends javax.swing.JPanel {

    private MainFrame mainFrame;
    private ManageLocationToGoDistributionDialog dialog;
    private List<LocationToGo> listLocationToGo;
    private List<Row> listRow;

    public ManageLocattionToGoDistributionPanel(MainFrame frame, ManageLocationToGoDistributionDialog dialog) {
        initComponents();
        this.mainFrame = frame;
        this.dialog = dialog;
        this.listLocationToGo = new ArrayList();

        for (LocationToGo ha : this.mainFrame.getCurrentCity().getListLocationToGo()) {
            LocationToGo han = new LocationToGo(ha.getName(), ha.getCity());
            han.setIsNew(false);
        ...
        for (String s : Data.tabLocation) {
            boolean old = false;
            for (LocationToGo ha : this.listLocationToGo) {
                if (ha.getName().equals(s)) {
                    old = true;
                    break;
                }
            }
            if (!old) {
                LocationToGo ha = new LocationToGo(s, this.mainFrame.getCurrentCity());
                ha.setIsNew(true);
                ha.setDeleted(true);
                this.listLocationToGo.add(ha);
            }
        }
    ...
    private void initMainPanel() {
        ...
        this.mainPanel.setPreferredSize(new Dimension(200, this.listLocationToGo.size() * 38));
        for (LocationToGo ha : this.listLocationToGo) {
            Row r = new Row(ha);
    ...
    private void okButtonActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_okButtonActionPerformed
        this.mainFrame.getCurrentCity().removeLocationToGo(this.listLocationToGo);
        this.mainFrame.getCurrentCityPanel().updateAgePanel();
  ...
  private class Row extends JPanel {
        ...
        private void boxEvent() {
            if (this.box.isSelected()) {
                this.loc.setDeleted(false);
                this.loc.setSaved(false);
            } else {
                this.loc.setDeleted(true);
                this.loc.setSaved(false);
            }
